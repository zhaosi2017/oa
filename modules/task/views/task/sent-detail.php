...
<div class="task-view">
    <p class="<?= $model->status!=2 && $model->status!=3 ? 'btn-group hidden-xs' : 'hide' ?>">
        <?= Html::a('任务信息', ['/task/task/sent-detail','id'=>$model->id], ['class' => 'btn btn-primary']) ?>
        <?= Html::a('执行反馈', ['/task/feedback/index','id'=>$model->id], ['class' =>  'btn btn-outline btn-default']) ?>
        <?= Html::a('收付款信息', ['/task/pay/index','id'=>$model->id], ['class' =>  'btn btn-outline btn-default']) ?>
        <?= Html::a('任务备注', ['/task/remark/index','id'=>$model->id], ['class' =>  'btn btn-outline btn-default']) ?>
    </p>
...
//                            $execute_price = $identity->company_id != $model['product']['company_id'] ? '******' : $model['execute']->price;
            ...
            <div class="form-group">
                ...
                <div class="pull-right">
                    <?= Html::a('发布',
                        Url::to(['/task/task/release','task_id' => $model->id]),
                        [
                            'class' => $model->status!=2 ? 'hide' : 'btn btn-primary btn-xs' ,
                            'data-method' => 'post',
                            'data' => ['confirm' => '你确定要发布该任务吗?']
                        ]) ?>

                    <?= Html::submitButton('作废', ['class' => ($model->status==2 || $model->status==3) ? 'btn btn-danger btn-xs' : 'hide' ,'onclick'=>'
                        layer.open({
                            title: "作废说明",
                            type: 1,
                            area: [\'35%\', \'35%\'], //宽高
                    ...
                    <?= Html::submitButton('任务撤销', ['class' => $model->status==4 ? 'btn btn-danger btn-xs' : 'hide' ,'onclick'=>'
                        layer.open({
                            title: "任务撤销说明",
                            type: 1,
                            area: [\'35%\', \'35%\'], //宽高
                    ...
                    <?= Html::submitButton('结算', ['class' => $model->status==5 ? 'btn btn-primary btn-xs' : 'hide' ,'onclick'=>'
                        layer.open({
                            title: "结算说明",
                            type: 1,
                            area: [\'35%\', \'35%\'], //宽高
                    ...
                    <?= Html::submitButton('验收不通过', ['class' => $model->status==5 ? 'btn btn-danger btn-xs' : 'hide' ,'onclick'=>'
                        layer.open({
                            title: "验收不通过说明",
                            type: 1,
                            area: [\'35%\', \'35%\'], //宽高
                    ...
                    <?= Html::submitButton('已完成', ['class' => $model->status==6 ? 'btn btn-danger btn-xs' : 'hide' ,'onclick'=>'
                        layer.open({
                            title: "完成说明",
                            type: 1,
                            area: [\'35%\', \'35%\'], //宽高
